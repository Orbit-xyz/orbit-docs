...
$$
\text{now} \ge \text{last\_pull} + \text{interval\_seconds} \;\lor\; \text{last\_pull} = 0
$$

$$
\text{amount\_per\_interval} \le \text{allowance}(\text{user}, \text{orbit}) \;\land\; \text{amount\_per\_interval} \le \text{balance}(\text{user})
$$
